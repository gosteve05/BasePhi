...
BigInt::BigInt(string x) //cannot be called implicitly!
{
	...
	if (x.size() == 0)
	{
		cout << "Not a number. Program will exit." << endl;
		exit(1);
	}
...
BigInt BigInt::operator*(const BigInt& right) const
{
	BigInt leftCopy(*this);
	BigInt rightCopy(right);
	BigInt ret = multHelper(leftCopy, rightCopy);
	ret.isNegative = (isNegative != right.isNegative);
 	return multHelper(leftCopy, rightCopy);
}

BigInt BigInt::operator&&(const BigInt& right) const //Slow multiplication
{
